...
三： MySQL 是怎么加行级锁的？
	...
	4： 非唯一索引等值查询
		...
		思考：当有一个事务持有间隙锁 (22, 39) 时，到底是什么情况下，可以让其他事务的插入  age = 39 记录的语句成功？又是什么情况下，插入 age = 39 记录时的语句会被阻塞？
...
3： 非唯一索引和主键索引的范围查询的加锁规则不同之处在于：
	唯一索引在满足一些条件的时候，索引的 next-key lock 退化为间隙锁或者记录锁。
	非唯一索引范围查询，索引的 next-key lock 不会退化为间隙锁和记录锁。

还有一件很重要的事情，在线上在执行 update、delete、select ... for update 等具有加锁性质的语句，一定要检查语句是否走了索引，如果是全表扫描的话，
