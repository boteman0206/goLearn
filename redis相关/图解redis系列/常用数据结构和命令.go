...
#常见命令
XADD：插入消息，保证有序，可以自动生成全局唯一 ID； XADD mymq * name xiaolin   * 表示让 Redis 为插入的数据自动生成一个全局唯一的 ID
XLEN ：查询消息长度；
...
消息队列
	XADD mymq * name xiaolin
		 * 表示让 Redis 为插入的数据自动生成一个全局唯一的 ID  1654254953808-0
...
XGROUP CREATE mymq group2 0-0    创建一个名为 group2 的消费组，0-0 表示从第一条消息开始读取。
...
XPENDING mymq group2  我们来查看一下 group2 中各个消费者已读取、但尚未确认的消息个数
XPENDING mymq group2 - + 10 consumer2   # 查看 group2 里 consumer2 已从 mymq 消息队列中读取了哪些消息
XACK mymq group2 1654256265584-0   一旦消息 1654256265584-0 被 consumer2 处理了，consumer2 就可以使用 XACK 命令通知 Streams，然后这条消息就会被删除。
