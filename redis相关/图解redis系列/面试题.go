...
十三：Redis 的大 key 如何处理？
	1：大 key 并不是指 key 的值很大，而是 key 对应的 value 很大。
		String 类型的值大于 10 KB；Hash、List、Set、ZSet 类型的元素的个数超过 5000个；
	2：大 key 会带来以下四种影响：
	...
	3、查找大key
		redis-cli -h 127.0.0.1 -p6379 -a "password" -- bigkeys  最好选择在从节点上执行该命令。因为主节点上执行时，会阻塞主节点；
		使用 SCAN 命令查找大 key
		使用 RdbTools 工具查找大 key
	4：删除大 key 这一个动作，我们要小心
		分批次删除
		异步删除（Redis 4.0版本以上）
